...
export default function SetupNumbersScreen() {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>🔢 Numbers Setup</Text>
        <View style={{ width: 60 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        {NUMBER_DATA.map((item) => {
          ...
          const displayName = custom?.customName ?? item.characterName;
          const faceUri     = custom?.customImageUri;
          return (
            <TouchableOpacity
              key={item.number}
              style={[styles.row, { borderLeftColor: item.accentColor }]}
              onPress={() => router.push(`/setup/number/${item.number}`)}
            >
              <View style={[styles.badge, { backgroundColor: item.bgColor }]}>
                <Text style={[styles.badgeText, { color: item.accentColor }]}>{item.number}</Text>
              </View>
